...
public:
    bool hasPath(vector<vector<int>>& maze, vector<int>& start, vector<int>& destination) {
        ...
        return recurse(maze, start, destination, visited);
    }
    bool recurse(vector<vector<int>>& maze, vector<int>& start, vector<int>& destination, set<vector<int>>& visited) {
        ...
        for(int i = 0; i < 4; i++) {
            ...
            if(res == destination || recurse(maze, res, destination, visited)) return true;
        }
        return false;
    }
